"""Layered TOML configuration loader for the Volnix framework.
...
producing a validated :class:`~volnix.config.schema.VolnixConfig`.
"""
...
class ConfigLoader:
    ...
    Layer order (later layers override earlier):
        ...
        4. Environment variables (``VOLNIX__section__key``)
    ...
    def load(self) -> VolnixConfig:
        """Load, merge, and validate all configuration layers.

        Returns:
            A fully resolved and validated :class:`VolnixConfig`.
        """
        config: dict[str, Any] = {}

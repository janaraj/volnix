"""Volnix CLI -- command interface for managing programmable worlds."""
...
from volnix.cli_helpers import (
    app_context,
    console,
    format_diff,
    format_entity_table,
...
def run(
    ...
    Examples:
      volnix run customer_support                  # compile + run
      volnix run customer_support --internal agents_team.yaml  # with internal team
      volnix run --world world_id --agents agents.yaml  # with external agents
      volnix run "Market analysis" --deliverable prediction --actors economist,analyst,strategist
    """
...
def _is_game_runner(runner: Any) -> bool:
    ...
    Matches :class:`OrchestratorRunner` produced by ``_setup_simulation``
    when the blueprint has ``game.enabled: true``. Both runners return
    different result shapes from ``.run()``, so the CLI uses this flag
    to dispatch formatting.
    """
    return type(runner).__name__ == "OrchestratorRunner"
...
def _format_run_result(result: Any, is_game: bool) -> str:
    ...
    OrchestratorRunner returns a GameResult object with ``.reason``
    and optional ``.winner``.
    """
...
async def _setup_simulation(volnix: Any, compiled_plan: Any) -> tuple[Any, Any, Any] | None:
    ...
    Dispatches on ``plan.game.enabled``:

    - Game blueprint → :class:`OrchestratorRunner` wrapping the
      :class:`GameOrchestrator`. The orchestrator was already configured
      via ``app.configure_game`` and bootstraps its first activation
      inside ``_on_start``.
    - Non-game blueprint → :class:`SimulationRunner`.
    ...
    agency = volnix.registry.get("agency")
    ...
    if game_def and getattr(game_def, "enabled", False):
        ...
        try:
            orchestrator = volnix.registry.get("game")
        except KeyError:
        ...
        if orchestrator is not None:
            ...
            console.print(
                f"  [green]Game mode: {game_def.mode}, "
    ...
    try:
        async with app_context() as volnix:
            ...
            if world_id:
                ...
                from volnix.paths import resolve_blueprint, sanitize_filename, user_blueprints_dir
            ...
            if serve:
                ...
                if port:
                    gw_config = volnix.gateway.config
                    gw_config.host = host or gw_config.host
                    gw_config.port = port
                await volnix.gateway.start_adapters()
                gw_cfg = volnix.config.gateway
            ...
            result = await volnix.end_run(run_id)
...
def serve(
    ...
    Examples:
      volnix serve customer_support                                       # world only
      volnix serve customer_support --agents external.yaml                # world + external agents
      volnix serve climate_research_station --internal agents_climate.yaml  # world + internal team
...
async def _serve_impl(
    ...
    try:
        async with app_context() as volnix:
                ...
                async def _compile_world() -> None:
                    try:
                        ...
                        if resolved:
                            ...
                            compiled_plan = await compiler.compile_from_nl(world)
                            from volnix.engines.world_compiler.plan_reviewer import (
                                PlanReviewer,
                            )
            ...
            http_adapter = volnix.gateway._adapters.get("http")
            if http_adapter:
                try:
...
def mcp(
    ...
    """Run Volnix as MCP stdio server for agent subprocess connections.
    ...
    Examples:
      volnix mcp world.yaml
      volnix mcp "Support team with email and tickets"
    """
...
async def _mcp_impl(world: str, settings: str | None) -> None:
    try:
        async with app_context() as volnix:
            ...
            await volnix.compile_and_run(plan)

            mcp_adapter = volnix.gateway._adapters.get("mcp")
...
def blueprints(
    ...
    from volnix.paths import list_blueprints, list_presets
...
async def _report_impl(run_id_str: str, fmt: str, output: Path | None) -> None:
    try:
        async with app_context() as volnix:
            ...
            run_record = await volnix.run_manager.get_run(RunId(run_id_str))
    ...
    try:
        ...
        async with app_context() as volnix:
            if gov_vs_ungov and len(runs) == 2:
                result = await volnix.diff_governed_ungoverned(runs[0], runs[1])
            else:
                result = await volnix.diff_runs(runs)
    ...
    try:
        async with app_context() as volnix:
            if resource == "entities":
                state = volnix.registry.get("state")
                if entity_type:
                ...
                actor_registry = volnix.actor_registry
                if actor_registry:
                    if actor_id:
                        from volnix.core.types import ActorId as AId
                ...
                policy_engine = volnix.registry.get("policy")
                ...
                responder = volnix.registry.get("responder")
...
async def _list_impl(resource: str, run_id: str | None, limit: int, fmt: str) -> None:
    try:
        async with app_context() as volnix:
            if resource == "runs":
                runs = await volnix.run_manager.list_runs(limit=limit)
                if fmt == "json":
                    print_json(runs)
                else:
                ...
                engines = volnix.registry.list_engines()
                if fmt == "json":
                ...
                artifacts = await volnix.artifact_store.list_artifacts(RunId(run_id))
                if fmt == "json":
    ...
    try:
        async with app_context() as volnix:
            if resource == "run":
                ...
                run_record = await volnix.run_manager.get_run(RunId(name))
                if run_record is None:
...
async def _replay_impl(run_id_str: str, tag: str | None) -> None:
    try:
        async with app_context() as volnix:
            ...
            source_run = await volnix.run_manager.get_run(RunId(run_id_str))
            if source_run is None:
                print_error(f"Run not found: {run_id_str}")
                raise typer.Exit(1)

            config_artifact = await volnix.artifact_store.load_artifact(
            ...
            event_log = await volnix.artifact_store.load_artifact(
            ...
            await volnix.end_run(new_run_id)
            print_success(f"Replay complete. New run: {new_run_id}")

            comparison = await volnix.diff_runs([source_run["run_id"], str(new_run_id)])
...
async def _setup_impl(provider: str) -> None:
    ...
    console.print("[bold]Volnix Setup Wizard[/bold]\n")
    ...
    console.print("\n[bold]Setup complete.[/bold] Run 'volnix check' to verify connectivity.")
...
async def _check_impl(run_tests: bool) -> None:
    ...
    console.print("[bold]Volnix System Check[/bold]\n")
...
async def _ledger_impl(
    ...
    try:
        async with app_context() as volnix:
            ...
            query = builder.build()
            entries = await volnix.ledger.query(query)
    ...
    try:
        async with app_context() as volnix:
            ...
            if submit_pr:
                ...
                console.print("  1. Fork https://github.com/volnix-project/profiles")
...
def config_cmd(
    ...
    if not export:
        ...
        for name in sorted(EXPORT_REGISTRY.keys()):
            console.print(f"  volnix config --export {name}")
        return
...
def attach(
    ...
    """Patch agent config to connect to Volnix."""
    import json

    from volnix.cli_exports.attach import _get_config_path, patch_config
    from volnix.cli_exports.templates import EXPORT_REGISTRY
    ...
    console.print(f"  Volnix URL: {url}")
    console.print(f"  Backup: {config_path}.volnix-backup")
...
def detach(
    ...
    from volnix.cli_exports.attach import _get_config_path, restore_config
...
async def _dashboard_impl(host: str, port: int, env: str) -> None:
    try:
        async with app_context(env=env) as volnix:
            ...
            run_count = len(await volnix.run_manager.list_runs())

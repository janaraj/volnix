    ...
    async def _start_animator_bridge(self, behavior: str) -> None:
        ...
        async def _on_committed_event(event: Any) -> None:
            ...
            if actor in ("system", "animator", "world_compiler"):
                return
            # F2 (P6.3-fix.3): in game mode, only trigger on PLAYER
            # actions. NPC actors (U03ATLAS02, U02VULCAN01, etc.) are
            # generated by the animator itself — re-triggering on them
            # creates an infinite loop. In non-game mode (empty
            # player_ids), trigger on all non-system actors (original
            # behavior preserved).

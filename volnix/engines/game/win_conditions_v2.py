"""Event-driven win condition handlers (Cycle B).

New handlers for the GameOrchestrator event-driven flow. The legacy
``win_conditions.py`` stays in place during the migration for the
legacy GameEngine/GameRunner path, and is deleted in Cycle B.10.

Handlers in this module read ``GameState`` (event_counter, terminated,
started_at) instead of ``RoundState`` (current_round, total_rounds).
They fire either on committed game tool events (natural win path) or
on timeout events from the orchestrator's failsafe timers (timeout
path). See ``volnix/engines/game/orchestrator.py`` for the calling
contract.
...
Two preserved handlers (compatible with the new context shape):
...
class WinConditionContextV2(BaseModel):
    """Context passed to event-driven win condition handlers.

    Replaces ``WinConditionContext`` from the legacy module. Key changes:
...
@runtime_checkable
class WinConditionHandlerV2(Protocol):
    """Protocol for event-driven win condition handlers."""

    async def check(self, ctx: WinConditionContextV2) -> WinResult | None:
...
class ScoreThresholdHandlerV2:
    ...
    Kept with the same name (plus V2 suffix) as the legacy class so
    competitive blueprints can migrate with zero YAML changes to their
    ``win_conditions`` block.
    """
...
class EliminationHandlerV2:
    """Eliminate players below a metric threshold. Last standing wins.

    Replaces the legacy ``EliminationHandler.elimination_round`` write
    with ``eliminated_at_event`` (reads ``ctx.game_state.event_counter``).
    """
...
WIN_CONDITION_HANDLER_REGISTRY_V2: dict[str, type] = {
    ...
    "score_threshold": ScoreThresholdHandlerV2,
    "elimination": EliminationHandlerV2,
}


class EventDrivenWinConditionEvaluator:
    ...
    def __init__(
        ...
        filtered: list[tuple[WinCondition, WinConditionHandlerV2]] = []
        for cond in conditions:
            handler_cls = WIN_CONDITION_HANDLER_REGISTRY_V2.get(cond.type)
            if handler_cls is None:
                logger.warning(
                    "EventDrivenWinConditionEvaluator: unknown win condition type %r — skipping",
                    cond.type,
                )
                continue
            if scoring_mode == "behavioral" and cond.type == "score_threshold":
                logger.info(
                    "EventDrivenWinConditionEvaluator: filtering out "

...
from volnix.core import BaseEngine, Event, ServiceId
...
class FeedbackEngine(BaseEngine):
    ...
    async def _ensure_initialized(self) -> None:
        ...
        if self._feedback_config.external_sync_enabled:
            from volnix.engines.feedback.drift import DriftDetector
            from volnix.engines.feedback.proposer import (
                ProfileUpdateProposer,
            )
            from volnix.engines.feedback.sync import (
                ExternalSyncChecker,
            )
    ...
    async def _record_annotation_ledger(self, service_id: str, text: str, author: str) -> None:
        ...
        if not hasattr(self, "_ledger") or self._ledger is None:
            return
        from volnix.ledger.entries import FeedbackAnnotationEntry
    ...
    async def _record_capture_ledger(
        ...
        if not hasattr(self, "_ledger") or self._ledger is None:
            return
        from volnix.ledger.entries import FeedbackCaptureEntry
    ...
    async def _record_promotion_ledger(
        ...
        if not hasattr(self, "_ledger") or self._ledger is None:
            return
        from volnix.ledger.entries import FeedbackPromotionEntry
    ...
    async def _record_sync_ledger(
        ...
        if not hasattr(self, "_ledger") or self._ledger is None:
            return
        from volnix.ledger.entries import FeedbackSyncEntry
    ...
    async def _record_sync_update_ledger(
        ...
        if not hasattr(self, "_ledger") or self._ledger is None:
            return
        from volnix.ledger.entries import FeedbackSyncUpdateEntry

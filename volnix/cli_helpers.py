"""Shared Rich formatting helpers for the Volnix CLI.
...
async def app_context(env: str = "development") -> AsyncIterator[Any]:
    """Yield a fully-started VolnixApp, stopping it on exit.

    Args:
        env: Configuration environment name. Maps to
            ``volnix.{env}.toml`` overlay file.
    ...
    if config.logging.llm_debug:
        _logging.getLogger("volnix.llm.router").setLevel(_logging.DEBUG)

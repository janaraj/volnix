"""Volnix SDK — connect to a running Volnix world.

Standalone HTTP client with zero internal Volnix imports.
Uses ``httpx`` to talk to the Volnix HTTP API.
...
class VolnixSDKError(Exception):
    """Base exception for Volnix SDK errors."""


class VolnixConnectionError(VolnixSDKError):
    """Cannot connect to the Volnix server."""


class VolnixAPIError(VolnixSDKError):
...
async def get_tool_manifest(
    ...
    """Fetch tool definitions from a running Volnix server.

    Args:
        url: Base URL of the Volnix server.
    ...
    Raises:
        VolnixConnectionError: If the server is unreachable.
        VolnixAPIError: If the server returns an error.
    """
    try:
...
async def execute_tool(
    ...
    """Execute a tool on a running Volnix server.
...
# ---------------------------------------------------------------------------
# VolnixClient — reusable async client
# ---------------------------------------------------------------------------


class VolnixClient:
    """High-level async client for a running Volnix world.
    ...
    async def __aenter__(self) -> VolnixClient:
        return self

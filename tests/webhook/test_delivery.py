...
async def test_successful_delivery():
    ...
    delivery = WebhookDelivery(max_retries=0, backoff_base=0.01, timeout=2.0)
...
async def test_4xx_no_retry():
    ...
    delivery = WebhookDelivery(max_retries=3, backoff_base=0.01, timeout=2.0)
    mock_response = AsyncMock()
    ...
    with patch("volnix.webhook.delivery.httpx.AsyncClient") as mock_cls:
        ...
        result = await delivery.send("http://agent:3000/hook", {"event": "test"})
...
async def test_connection_error_retries():
    ...
    delivery = WebhookDelivery(max_retries=2, backoff_base=0.01, timeout=1.0)
...
async def test_max_retries_exhausted():
    ...
    delivery = WebhookDelivery(max_retries=1, backoff_base=0.01, timeout=1.0)

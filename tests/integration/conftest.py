                    ...
                    entities.append(
                        {
                            ...
                            "labelIds": [["INBOX", "UNREAD"], ["INBOX"], ["SENT"], ["INBOX"]][
                                idx % 4
                            ],
...
def _generate_generic_entities(
    ...
    templates: dict[str, callable] = {
        ...
        "subscription": lambda idx: {
            ...
            "customer": f"cus_{(idx % 5) + 1:03d}",
            "status": "active",
            "current_period_end": 1800000000 + idx,
        ...
        "reversal": lambda idx: {
            ...
            "amount": 500,
            "status": "succeeded",
            "charge": f"ch_{(idx % 5) + 1:03d}",
            "payment_intent": f"pi_{(idx % 5) + 1:03d}",
        },
        "balance": lambda idx: {
            "id": f"bal_{idx:03d}",
            "available": (idx + 1) * 10000,
            "pending": idx * 500,
            "currency": "usd",
        },
        "web_site": lambda idx: {
            "id": f"site_{idx:03d}",
            "domain": f"site{idx}.acme.com",
            "name": f"Site {idx}",
        ...
        "pull_request": lambda idx: {
            ...
            "body": f"PR body {idx}",
            "state": "open",
            "head": {"ref": f"feature-{idx}", "sha": f"abc{idx}"},
        ...
        "commit": lambda idx: {
            ...
            "author": {
                "name": "Dev",
                "email": "dev@test.com",
                "date": f"2026-03-{idx + 1:02d}T00:00:00Z",
            },
            "committer": {
                "name": "Dev",
                "email": "dev@test.com",
        ...
        "attendee": lambda idx: {
            "id": f"att_{idx:03d}",
            "event_id": "evt_001",
            "email": f"attendee{idx}@test.com",
            "responseStatus": "needsAction",
        },
        "invoice": lambda idx: {
            "id": f"in_{idx:03d}",
            "customer": "cus_001",
            "amount_due": (idx + 1) * 500,
            "currency": "usd",
            "status": "draft",
            "created": 1700000000 + idx,
        },
        ...
        "review": lambda idx: {
            "id": f"review_{idx:03d}",
            "pull_number": 100 + idx,
            "user": {"login": f"reviewer{idx}"},
            "state": "COMMENTED",
            "body": f"Review comment {idx}",

"""Tests for the event-driven win condition handlers (Cycle B).

Exercises ``volnix/engines/game/win_conditions_v2.py`` — the new set of
handlers that read ``GameState`` / ``state_engine`` instead of
``RoundState``. Each handler tested in isolation, plus the
``EventDrivenWinConditionEvaluator`` filter behavior between behavioral
...
from volnix.engines.game.win_conditions_v2 import (
    WIN_CONDITION_HANDLER_REGISTRY_V2,
    AllBudgetsExhaustedHandler,
    DealClosedHandler,
    DealRejectedHandler,
    EliminationHandlerV2,
    EventDrivenWinConditionEvaluator,
    MaxEventsExceededHandler,
    ScoreThresholdHandlerV2,
    StalemateHandler,
    WallClockElapsedHandler,
    WinConditionContextV2,
)
...
class TestEventDrivenWinConditionEvaluator:
    ...
    async def test_behavioral_mode_filters_out_score_threshold(self, caplog):
        ...
        ev = EventDrivenWinConditionEvaluator(conditions, scoring_mode="behavioral")
    ...
    async def test_competitive_mode_keeps_score_threshold(self):
        ...
        ev = EventDrivenWinConditionEvaluator(conditions, scoring_mode="competitive")
        state = AsyncMock()
        state.query_entities = AsyncMock(return_value=[])
        result = await ev.check(
            _make_scores(["a", "b"], {"a": 85.0, "b": 50.0}),
...
class TestRegistryRegistration:
    ...
    def test_registry_contains_all_new_types(self):
        ...
        assert set(WIN_CONDITION_HANDLER_REGISTRY_V2.keys()) == expected

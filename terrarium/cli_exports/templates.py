...
def env_vars(url: str, tools: list[dict[str, Any]]) -> str:
    ...
    M4 fix: only outputs TERRARIUM_URL and MCP URL.
    Per-service API URLs (SLACK_API_URL etc.) are Phase 2 (Path 1A).
    """
    return (
        f"export TERRARIUM_URL={url}\n"
        f"export TERRARIUM_MCP_URL={url}/mcp"
    )
...
async with TerrariumClient(url="{url}") as terra:
...
def typescript_sdk(url: str, tools: list[dict[str, Any]]) -> str:
    ...
    return f'''\
// Terrarium HTTP API
const TERRARIUM_URL = "{url}";

// List tools
const tools = await fetch(`${{TERRARIUM_URL}}/api/v1/tools?format=openai`).then(r => r.json());

// Call a tool
const result = await fetch(`${{TERRARIUM_URL}}/api/v1/actions/email_send`, {{
  method: "POST",
